...
import { CobrandingCase, exportNotebookToMarkdown, formatCaseToMarkdown } from '../types';
...
export const Notebook: React.FC<NotebookProps> = ({ cases, brandName, onDelete, onBack, hasPendingReview }) => {
  ...
  const handleExport = () => {
    const text = exportNotebookToMarkdown(cases, brandName || "Research");
    const blob = new Blob([text], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${brandName || 'Brand'}_Research_Notebook.md`;
  ...
  if (cases.length === 0) {
    return (
      <div className="max-w-4xl mx-auto mt-10 text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
         ...
         <h2 className="text-xl font-bold text-gray-900">Your Notebook is Empty</h2>
         <p className="text-gray-500 mb-8">Start a new research to collect cases.</p>
         <button onClick={onBack} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700">
            {hasPendingReview ? 'Return to Review' : 'Start New Research'}
         </button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-20 z-40">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Research Notebook</h2>
          <p className="text-sm text-gray-500">{cases.length} cases collected {brandName ? `for ${brandName}` : ''}</p>
        </div>
          ...
          <div key={index} className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 group relative">
            ...
            <div className="space-y-6 text-gray-700 text-sm leading-relaxed">
              <div className="p-5 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <p className="text-gray-600 mb-3">{data.projectName}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-5 bg-purple-50 rounded-xl border border-purple-100 flex flex-col">
                  ...
                  <p className="text-purple-800 flex-grow">{data.insight}</p>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 mt-4">
                 ...
                 <div className="flex flex-wrap gap-2">
                  {data.sourceUrls.map((url, i) => (
                      <a key={i} href={url} target="_blank" rel="noreferrer" className="px-3 py-1 bg-gray-50 border border-gray-200 text-gray-600 rounded-md text-xs hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-colors truncate max-w-[200px]">
