  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 mt-24 px-4 animate-fade-in-up">
      <div className="bg-white/90 backdrop-blur-xl rounded-[40px] shadow-2xl border border-white/50 overflow-hidden mb-8">
        ...
        <div className="bg-gradient-to-r from-[#b5004a] to-purple-700 px-8 py-8 flex flex-col md:flex-row justify-between items-center relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white opacity-5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl"></div>
          
          <div className="relative z-10 text-center md:text-left mb-6 md:mb-0">
            <h2 className="text-3xl font-bold text-white tracking-tight">搜寻简报</h2>
            <p className="text-pink-100 text-sm mt-1 font-medium">请审核并在入库前筛选...</p>
          </div>
          <div className="flex gap-4 relative z-10">
             <button onClick={onDiscardAll} className="px-6 py-3 text-pink-100 hover:text-white font-bold text-sm bg-white/10 rounded-xl hover:bg-white/20 transition-colors backdrop-blur-sm">放弃</button>
             <button 
                onClick={handleConfirm}
                className="px-8 py-3 bg-white text-[#b5004a] font-bold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all text-sm flex items-center gap-2"
             >
               <span>入库 {selectedIndices.length} 个案例</span>
           ...
           <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto custom-scrollbar">
             {metadata?.groundingChunks?.map((chunk, i) => (
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer"
                  className="flex items-center px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-600 hover:text-[#b5004a] hover:border-[#b5004a]/20 hover:shadow-sm transition-all"
                >
              ...
              {newCases.map((c, idx) => {
                 ...
                 return (
                  <div 
                    ...
                    className={`relative bg-white rounded-3xl p-8 cursor-pointer transition-all duration-300 border-2 group ${selectedIndices.includes(idx) ? 'border-[#b5004a] shadow-xl scale-[1.01] z-10' : 'border-transparent opacity-70 hover:opacity-100 hover:scale-[1.005]'}`}
                  >
                    {/* Selection Checkbox */}
                    <div className={`absolute top-6 right-6 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${selectedIndices.includes(idx) ? 'bg-[#b5004a] border-[#b5004a] scale-110' : 'border-slate-200 bg-slate-50'}`}>
                    ...
                    <div className="mt-2 pr-12">
                      ...
                      <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-slate-100">
                       ...
                       <div className="mt-6 pt-4 border-t border-slate-100">
                         <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">纯文本预览</h4>
